sustIndex <- function(db,
  ...
  pops <- select(db$POP_EVAL, c('EVALID', 'ESTN_METHOD', 'CN', 'GROWTH_ACCT', 'END_INVYR', 'EVAL_TYP')) %>%
    ...
    left_join(select(db$PLOT, PLT_CN, REMPER, PREV_PLT_CN), by = 'PLT_CN') %>%
  ...
  suppressWarnings({
    ...
    if (Sys.info()['sysname'] == 'Windows'){
      ...
      out <- parLapply(cl, X = names(plts), fun = sustIndexHelper1, plts, db, grpBy, aGrpBy, byPlot, minLive)
      #stopCluster(cl) # Keep the cluster active for the next run
    } else { # Unix systems
      out <- mclapply(names(plts), FUN = sustIndexHelper1, plts, db, grpBy, aGrpBy, byPlot, minLive, mc.cores = nCores)
    }
  })
  ...
  if (byPlot){
    ...
    if (totals) {
      tOut <- tOut %>%
        select(grpBy, SUST_INDEX, TPA_RATE, BAA_RATE, CHNG_TPA, CHNG_BAA, PREV_TPA, PREV_BAA,
