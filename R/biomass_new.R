bioStarter_new <- function(x,
  ...
  suppressWarnings({
    ...
    if (Sys.info()['sysname'] == 'Windows'){
      ...
      out <- parLapply(cl, X = names(plts), fun = bioHelper1_new, plts,
                       db[names(db) %in% c('COND', 'TREE')],
                       grpBy, aGrpBy, byPlot, component)
      #stopCluster(cl) # Keep the cluster active for the next run
    } else { # Unix systems
      out <- mclapply(names(plts), FUN = bioHelper1_new, plts,
                      db[names(db) %in% c('COND', 'TREE')],
  ...
  if (byPlot){
    ...
    suppressWarnings({
      ## Compute estimates in parallel -- Clusters in windows, forking otherwise
      if (Sys.info()['sysname'] == 'Windows'){
        out <- parLapply(cl, X = names(popState), fun = bioHelper2_new, popState, a, t, grpBy, aGrpBy, method)
        stopCluster(cl)
      } else { # Unix systems
        out <- mclapply(names(popState), FUN = bioHelper2_new, popState, a, t, grpBy, aGrpBy, method, mc.cores = nCores)
      }
    })
    ## back to dataframes
    out <- unlist(out, recursive = FALSE)
...
biomass_new <- function(db,
  ...
  out <- lapply(X = iter, FUN = bioStarter_new, db,
      ...
      if (variance){
        ...
        tOut <- tOut %>%
          select(grpBy, "BIO_ACRE","CARB_ACRE", "BIO_ACRE_SE",
                 "CARB_ACRE_SE","nPlots_TREE","nPlots_AREA")
      }
