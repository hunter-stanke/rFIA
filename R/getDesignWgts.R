...
getDesignInfo <- function(db,
  ...
  ## Type must exist
  if (!c(class(type) == 'character')) {
    stop('`type` must be a character vector. Please choose one of (or a combination of): `ALL`, `CURR`, `VOL`, `GROW`, `MORT`, `REMV`, `CHANGE`, `DWM`,` REGEN`.')
  }
  type <- unique(stringr::str_to_upper(type))
  if (sum(type %in% c('ALL','CURR','VOL','GROW','MORT', 'REMV','CHANGE','DWM','REGEN')) < length(type)) {
    bad.type = type[!c(type %in% c('ALL','CURR','VOL','GROW','MORT', 'REMV','CHANGE','DWM','REGEN'))]
    stop(paste0("Don't recognize `type`: ", paste(as.character(bad.type), collapse = ', '), ". Please choose one of (or a combination of): `ALL`, `CURR`, `VOL`, `GROW`, `MORT`, `REMV`, `CHANGE`, `DWM`,` REGEN`."))
  }
  ...
  evals <- db$POP_EVAL %>%
    ## Slim it down
    select(EVAL_CN = CN, STATECD, YEAR = END_INVYR, EVALID) %>%
    ## Join eval type
    left_join(select(db$POP_EVAL_TYP, EVAL_CN, EVAL_TYP), by = 'EVAL_CN') %>%
    filter(!is.na(EVAL_TYP))
  ...
  if ( !is.null(evalid) ) {
    ...
    ## Subset evals
    evals <- filter(evals, EVALID %in% evalid)
  ...
  ## If EVALID not given, then subset by type. EVALID does this automatically
    ...
    ## Subset evals
    evals <- filter(evals, EVAL_TYP %in% paste0('EXP', type))
  ...
  strata <- evals %>%
    ...
    left_join(select(db$POP_ESTN_UNIT, ESTN_UNIT_CN = CN,
                     P1PNTCNT_EU, AREA_USED, EVAL_CN), by = 'EVAL_CN') %>%
    ## Join stratum
    left_join(select(db$POP_STRATUM, ESTN_UNIT_CN,
    ...
    mutate(STRATUM_WGT = (P1POINTCNT / P1PNTCNT_EU)) %>%
    ## Join plots to stratum
    left_join(select(db$POP_PLOT_STRATUM_ASSGN, PLT_CN, STRATUM_CN,
    ...
    select(STATECD, YEAR, EVAL_TYP, EVALID, ESTN_UNIT_CN, AREA_USED, STRATUM_CN, STRATUM_WGT, pltID, PLT_CN) %>%
    distinct()
