...
bioStarter_old <- function(x,
  ...
  suppressWarnings({
    ...
    if (Sys.info()['sysname'] == 'Windows'){
      ...
      out <- parLapply(cl, X = names(plts), fun = bioHelper1_old, plts,
                       db[names(db) %in% c('COND', 'TREE')],
                       grpBy, aGrpBy, byPlot)
      #stopCluster(cl) # Keep the cluster active for the next run
    } else { # Unix systems
      out <- mclapply(names(plts), FUN = bioHelper1_old, plts,
                      db[names(db) %in% c('COND', 'TREE')],
  ...
  if (byPlot){
    ...
    suppressWarnings({
      ## Compute estimates in parallel -- Clusters in windows, forking otherwise
      if (Sys.info()['sysname'] == 'Windows'){
        out <- parLapply(cl, X = names(popState), fun = bioHelper2_old, popState, a, t, grpBy, aGrpBy, method)
        stopCluster(cl)
      } else { # Unix systems
        out <- mclapply(names(popState), FUN = bioHelper2_old, popState, a, t, grpBy, aGrpBy, method, mc.cores = nCores)
      }
    })
    ## back to dataframes
    out <- unlist(out, recursive = FALSE)
...
biomass_old <- function(db,
  ...
  ## Run the main portion
  out <- lapply(X = iter, FUN = bioStarter_old, db,
                grpBy_quo = grpBy_quo, polys, returnSpatial,
